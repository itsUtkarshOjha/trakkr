...
import { getTomorrowWorkoutsAndUsers } from "./controllers/workout.controller";
...
const date = new Date();
date.setHours(15, 54, 0, 0);

const job = scheduleJob(date, async () => {
  const tomorrowWorkouts = await getTomorrowWorkoutsAndUsers();
  for (const workout of tomorrowWorkouts) {
    const notificationHTML = generateNotificationBody(
      workout.user.firstName,
      workout.workoutName
    );
    const notificationBody: NotificationBody = {
      html: notificationHTML,
      emailId: workout.user.emailId,
    };
